# webマンガサイト 仕様書

複数の出版社・Web マンガサイトが掲載している「読み切り」作品を横断的に一覧できる
キュレーションサイトの仕様を定める。
ビジュアルデザインの詳細は [002_design.md](./002_design.md) を参照する。

## 1. 概要

### 1.1 目的

読者が複数の Web マンガサイトを個別に巡回しなくても、各サイトが公開している
読み切り作品をまとめて発見できるようにする。

### 1.2 提供価値

- 読み切り作品をサムネイル付きのカード一覧で横断閲覧できる
- カードから直接、各サイトの作品ページへワンクリックで遷移できる
- ジャンル投票によって作品にジャンルタグが付き、ジャンルで絞り込める
- お気に入り・既読を記録し、読んだ作品を管理できる

### 1.3 対象範囲外

- 有料課金のみで読める作品、アプリ限定配信の作品は対象外とする
- 本サイト自体の会員登録・ログイン機能は当面実装しない
  （`isLogin` / `loginAccountType`（[4.1](#41-サイトマスタsitesjson)）は掲載サイト側の属性であり、本サイトのログインとは無関係）
- 検索機能・並び替え機能は初期リリースでは対象外とする（[9. 今後の課題](#9-今後の課題)）

## 2. 用語定義

| 用語           | 意味                                                                 |
| -------------- | -------------------------------------------------------------------- |
| 掲載サイト     | 出版社・事業者が運営する Web マンガサイト（例: 少年ジャンプ+）       |
| 作品           | 掲載サイトが公開する読み切り 1 話分のコンテンツ                      |
| 既読           | ユーザーが作品を読了したと本サイトが推測した状態                     |
| ジャンル投票   | 既読になった作品に対し、ユーザーがジャンルを投票する操作              |

## 3. 対象サイトの選定方針

掲載対象は
[ABJ マーク発行先一覧](https://aebs.or.jp/archives/001/202607/ABJmark_Whitelist.pdf)
（一般社団法人ABJ 発行、2026 年 07 月 24 日時点で 1029 サービス・267 事業者）に
掲載されている事業者が運営するサービスに限定する。ABJ マークは正規の電子書店・
マンガアプリであることを示す認証マークであり、これを選定基準とすることで
海賊版サイトへの誘導を排除する。

選定手順:

1. ABJ マーク発行先一覧から事業者・サービスを確認する
2. その中から、無料で読み切り作品を Web ブラウザ上に掲載しているサービスを選ぶ
   （例: 少年ジャンプ+、マンガクロス、コミック DAYS、くらげバンチ、トーチ web、
   web アクション、COMIC FUZ、マンガ Park、サンデーうぇぶり、マンガワン など。
   これらは選定基準の例示であり、掲載対象の全量ではない）
3. アプリ限定でブラウザ版が無いサービス、読み切りの掲載が確認できないサービスは除外する
4. ABJ マーク発行先一覧に掲載が無いサービスは対象としない

## 4. データ設計

作品・サイトの情報は DB を持たず、静的な JSON ファイルで管理する。

### 4.1 サイトマスタ（sites.json）

| フィールド          | 型                | 説明                                                                                   |
| ------------------- | ----------------- | -------------------------------------------------------------------------------------- |
| `id`                 | string             | サイト ID（例: `"shonenjumpplus"`）                                                    |
| `name`               | string             | サービス名（例: `"少年ジャンプ+"`）                                                    |
| `publisher`          | string             | 出版社・運営事業者名（例: `"株式会社集英社"`）                                         |
| `developer`          | string[]           | 開発元（配信システムを開発する事業者。複数の場合を考慮し配列とする）                   |
| `editorialDept`      | string[]           | 編集部名（複数編集部が横断的に運営する場合を考慮し配列とする）                         |
| `updateFrequency`    | object              | 更新頻度。自然言語の文字列ではなく構造化して持つ（[4.1.1](#411-updatefrequency-の構造)）|
| `type`               | string             | Web マンガサイトの種別（例: `"出版社型"`, `"書店型"` など）                            |
| `isLogin`            | boolean            | ログイン機能の有無                                                                     |
| `loginAccountType`   | string[]           | ログイン可能なアカウント種別（例: `"コミチ"`, `"オリジナル"`, `"集英社ID"`）。`isLogin` が `false` の場合は空配列とする |
| `isPurchase`         | boolean            | 作品の購入が可能か                                                                     |
| `isRental`           | boolean            | 作品のレンタルが可能か                                                                 |
| `isSubscribe`        | boolean            | 定期購読（サブスクリプション）が可能か                                                 |
| `hasApp`             | boolean            | スマートフォンアプリが存在するか                                                       |
| `saasBrand`          | string \| null     | 利用している配信 SaaS のブランド名（例: `"GigaViewer"`, `"コミチ+"`）。自社独自システムの場合は `null` |
| `description`        | string             | サイトの説明文                                                                         |
| `url`                | string             | サイトトップページの URL                                                               |
| `faviconUrl`         | string             | favicon 画像 URL                                                                        |
| `abjNo`              | string             | 対応する ABJ マーク管理番号（例: `"ABJ 10921006"`）                                    |

#### 4.1.1 `updateFrequency` の構造

| フィールド          | 型                                                    | 説明                                                                 |
| ------------------- | ------------------------------------------------------ | -------------------------------------------------------------------- |
| `unit`               | `"day"` \| `"week"` \| `"month"` \| `"irregular"`       | 更新周期の単位。不定期の場合は `"irregular"` とし、他フィールドは `null` とする |
| `interval`           | number \| null                                          | `unit` を何回ごとに繰り返すか（例: `unit: "week", interval: 2` は隔週） |
| `timesPerInterval`   | number \| null                                          | 1 インターバルあたりの更新回数（例: 週2回更新なら `2`）               |
| `daysOfWeek`         | string[] \| null                                        | 更新曜日が固定の場合のみ指定（例: `["mon", "thu"]`）                  |

例:

- 毎日更新: `{ "unit": "day", "interval": 1, "timesPerInterval": 1, "daysOfWeek": null }`
- 週2回（月・木）更新: `{ "unit": "week", "interval": 1, "timesPerInterval": 2, "daysOfWeek": ["mon", "thu"] }`
- 隔週更新: `{ "unit": "week", "interval": 2, "timesPerInterval": 1, "daysOfWeek": null }`
- 不定期: `{ "unit": "irregular", "interval": null, "timesPerInterval": null, "daysOfWeek": null }`

```json
{
  "id": "shonenjumpplus",
  "name": "少年ジャンプ+",
  "publisher": "株式会社集英社",
  "developer": ["株式会社集英社"],
  "editorialDept": ["少年ジャンプ+編集部"],
  "updateFrequency": {
    "unit": "day",
    "interval": 1,
    "timesPerInterval": 1,
    "daysOfWeek": null
  },
  "type": "出版社型",
  "isLogin": true,
  "loginAccountType": ["集英社ID"],
  "isPurchase": true,
  "isRental": false,
  "isSubscribe": false,
  "hasApp": true,
  "saasBrand": "GigaViewer",
  "description": "『ONE PIECE』などを擁するジャンプが送る、Web発マンガアプリ。",
  "url": "https://shonenjumpplus.com/",
  "faviconUrl": "https://shonenjumpplus.com/favicon.ico",
  "abjNo": "ABJ 10921006"
}
```

### 4.2 作品マスタ（works.json）

読み切り作品を格納する巨大な JSON ファイル。1 要素が 1 作品に対応する。

| フィールド      | 型       | 説明                                                   |
| --------------- | -------- | ------------------------------------------------------ |
| `id`            | string   | 作品 ID                                                 |
| `title`         | string   | 作品タイトル                                            |
| `author`        | string   | 作者名                                                  |
| `thumbnailUrl`  | string?  | サムネイル画像 URL。無い場合はプレースホルダーを表示     |
| `siteId`        | string   | 掲載サイト ID（`sites.json` の `id` を参照）             |
| `workUrl`       | string   | 作品ページへの直接リンク                                |
| `publishedAt`   | string   | 掲載日（ISO 8601）                                      |
| `genres`        | string[] | 投票により確定した上位ジャンル ID（表示用キャッシュ）    |

```json
{
  "id": "wj-0001234",
  "title": "サンプル読み切り",
  "author": "作者名",
  "thumbnailUrl": "https://shonenjumpplus.com/.../thumb.jpg",
  "siteId": "shonenjumpplus",
  "workUrl": "https://shonenjumpplus.com/episode/...",
  "publishedAt": "2026-07-20",
  "genres": ["genre-battle", "genre-comedy"]
}
```

### 4.3 ジャンルマスタ（genres.json）

| フィールド | 型     | 説明                        |
| ---------- | ------ | --------------------------- |
| `id`       | string | ジャンル ID                 |
| `name`     | string | ジャンル名（例: `"バトル"`）|

### 4.4 ユーザー状態データ

作品・サイトの静的データとは別に、以下の状態を扱う。

| 状態         | 保存先                     | 補足                                                       |
| ------------ | -------------------------- | ------------------------------------------------------------ |
| お気に入り   | ブラウザ localStorage      | 端末内のみで完結する。作品 ID の配列として保持する            |
| 既読         | ブラウザ localStorage      | [5.2](#52-既読判定とジャンル投票フロー) の推測ロジックで記録  |
| ジャンル投票 | サーバー側の集計ストア      | `POST /api/works/{workId}/vote` 等で送信し、集計結果を `genres` に反映する。保存先は [8. 技術スタック](#8-技術スタック)を参照。失敗時は [6.4](#64-エラー表示--空状態) のエラー表示を行う |

## 5. 主要機能

### 5.1 読み切り一覧・フィルタ

トップページに全作品をカードグリッドで表示する。ジャンル選択チップ（複数選択可）で
絞り込み、該当作品が 0 件の場合は空状態表示を行う。

### 5.2 既読判定とジャンル投票フロー

1. ユーザーが読み切りカードをクリックすると、`workUrl` を新規タブで開く
2. 元タブに戻ってきたことを `visibilitychange` で検知し、かつ一定時間
   （目安 10 秒以上）経過していた場合に「読了した可能性が高い」とみなす
3. 既読フラグを立て、まだ未投票の作品であればジャンル投票モーダルを表示する
4. モーダルで「投票する」または「スキップ」を選択する

既読判定はあくまでヒューリスティックであり、他サイト上での実際の読了を
保証するものではない。

### 5.3 お気に入り

カード上の星アイコンで作品をお気に入り登録・解除できる。

### 5.4 掲載サイト一覧

参加している掲載サイトの出版社・更新頻度・編集部・説明文を一覧できるサブ画面を
設ける。各項目からサイトトップページへ新規タブで遷移できる（[6.3](#63-掲載サイト一覧画面)）。

## 6. 画面仕様

[002_design.md 4 章「コンポーネント仕様」](./002_design.md) が対象とする UI 要素は
以下の各画面に登場する。

### 6.1 トップページ（読み切り一覧）

- ヘッダー（[4.6](./002_design.md)）
- ジャンル絞り込みチップ（[4.4](./002_design.md)、複数選択可、選択状態は投票モーダルと共通デザイン）
- 読み切りカードグリッド（[4.1](./002_design.md)）
  - カード内にジャンルバッジ（[4.2](./002_design.md)、得票上位 3 件）を表示
  - サムネイルが無い、または表示できない場合はプレースホルダー（[4.8](./002_design.md)）
  - 既読済みの作品は専用スタイルで区別する
  - お気に入りボタンはカード内に配置する
- フィルタ結果が 0 件の場合は空状態表示（[4.7](./002_design.md)）
- フッター（[4.6](./002_design.md)）

### 6.2 ジャンル投票モーダル

[5.2](#52-既読判定とジャンル投票フロー) のフローで表示する（[4.5](./002_design.md)）。

- 対象作品のサムネイルとタイトル
- ジャンル選択チップ一覧（[4.4](./002_design.md)）
- 「スキップ」（secondary）・「投票する」（primary）ボタン（[4.3](./002_design.md)）
  「投票する」はジャンル未選択の間 disabled とする

### 6.3 掲載サイト一覧画面

[5.4](#54-掲載サイト一覧) の内容を表示するサブ画面。サイトごとに次を表示する。

- サービス名・favicon
- 出版社
- 編集部
- 更新頻度
- 種別（`type`）
- 説明文
- 利用可否バッジ（`isPurchase` / `isRental` / `isSubscribe` / `hasApp` に対応する、購入・レンタル・
  定期購読・アプリの有無を示す小さなバッジ）
- サイトへのリンク（新規タブ遷移）

`developer` / `isLogin` / `loginAccountType` / `saasBrand` はサイトマスタの属性として保持するのみで、
当面この画面には表示しない。

カードの見た目は読み切りカードと同じデザイン原則
（角張った輪郭・ソリッドな枠線・ベタ塗りの影、[002_design.md 2 章](./002_design.md)）に従う。

### 6.4 エラー表示 / 空状態

- ジャンル投票 API 失敗時はエラー表示（[4.7](./002_design.md)）を行う
- フィルタ結果が 0 件の場合は空状態表示（[4.7](./002_design.md)）を行う

## 7. 非機能要件

- **コンプライアンス**: 掲載対象は [3 章](#3-対象サイトの選定方針)の基準に従い
  ABJ マーク発行先に限定し、海賊版サイトへの誘導は行わない
- **画像利用**: サムネイルは各掲載サイトが公開している画像を利用する。
  ホットリンクできない場合はプレースホルダー（[4.8](./002_design.md)）を表示する
- **アクセシビリティ**: [002_design.md 5 章](./002_design.md)に準拠する
- **データ更新運用**: `sites.json` / `works.json` の更新方法・更新頻度は実装フェーズで検討する
- **配信方式**: `works.json` の肥大化に伴う配信方法（分割・キャッシュ等）は実装フェーズで検討する

## 8. 技術スタック

| 分類           | 選定                    |
| -------------- | ----------------------- |
| フレームワーク | Next.js 16               |
| 実行環境       | Cloudflare Workers       |
| スタイリング   | CSS Modules + PostCSS    |
| アイコン       | react-icons              |
| データベース   | 用いない（[4 章](#4-データ設計)の通り静的 JSON で管理する） |

- Next.js は Cloudflare Workers 上で動作させる想定とする
  （実装には `@opennextjs/cloudflare` 等のアダプタを用いる。詳細は実装フェーズで確定する）
- `sites.json` / `works.json` / `genres.json` は静的アセットとして Workers から配信する
- デザイントークン（[002_design.md 3 章](./002_design.md)）は CSS Custom Properties として定義し、
  CSS Modules から参照する
- お気に入りの星、外部リンクなどのアイコン表示には react-icons を用いる
- ジャンル投票の集計（[4.4 ユーザー状態データ](#44-ユーザー状態データ)）は DB を持たない方針のため、
  Cloudflare KV 等の軽量なキーバリューストアでの実装を想定する（実装フェーズで確定する）

## 9. 今後の課題

- 検索機能・並び替え機能の追加
- ユーザーアカウントの導入（複数端末間でのお気に入り・既読の同期）
- 掲載サイト・作品データの自動収集・更新の仕組み
